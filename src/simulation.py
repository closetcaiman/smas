...
from genome import make_starting_genome, cross_genomes
...
def perform_agent_actions(region: Region):
    reproducing_agents = [agent for agent in region.agents if agent.wants_action() == Action.REPRODUCE]
...
def migrate_agents(current_region: Region, migrating_agents: List[Agent]):
    for agent in migrating_agents:
        available_regions = [r for r in current_region.neighbors if current_region.migrate_out_cost + r.migrate_in_cost < agent.energy]
        if len(available_regions) > 0:
            selected_region = random.choice(available_regions)
            agent.energy -= current_region.migrate_out_cost + selected_region.migrate_in_cost
...
def feed_agents(region: Region, eating_agents: List[Agent]):
    for agent in eating_agents:
        for pref in agent.genome.preferred_food.value:
            ...
            elif pref == Food.FRUIT and agent.genome.size.value >= 40 and region.fruit_amount > 0:
